  ...
  return (
    <div className="max-w-md mx-auto h-full px-4 pb-12">
      ...
      <div className="flex gap-2 mb-6 px-2">
        ...
        <button className="bg-white p-3 rounded-xl shadow-sm text-gray-500">
          <SlidersHorizontal size={20} />
        </button>
      </div>

      <div className="flex justify-between items-center mb-4 px-2">
        <p className="text-sm font-bold text-gray-500">{filteredHistory.length} Products Found</p>
        {history.length > 0 && (
          <button 
        ...
        <div className="space-y-4">
          {filteredHistory.map((scan) => {
            ...
            return (
              ...
              >
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-white p-2 border border-gray-100 shrink-0">
                  <img src={product.image} alt={product.name} className="w-full h-full object-contain" />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-sm truncate">{product.name}</h4>
                  <p className="text-[10px] text-gray-400 uppercase tracking-wider font-bold">{product.brand}</p>
