  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center h-screen p-8 text-center">
        ...
        <p className="text-gray-500 mb-8">We couldn't find the product you scanned in our database.</p>
    ...
    <div className="bg-background min-h-screen pb-24">
      <div className="relative h-72 bg-white">
        <div className="absolute top-6 left-6 z-10">
          <button onClick={() => navigate(-1)} className="p-2 bg-white/80 rounded-full shadow-md backdrop-blur-sm">
            <ChevronLeft size={24} />
          </button>
        </div>
        <div className="absolute top-6 right-6 z-10">
          <button className="p-2 bg-white/80 rounded-full shadow-md backdrop-blur-sm">
            <Share2 size={24} />
          </button>
        </div>
        <img 
      ...
      <div className="px-6 -mt-10 relative z-20">
          ...
          <Card className="mb-6 p-6">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <h2 className="text-2xl font-extrabold text-text">{product.name}</h2>
              </div>
            ...
            <div className="flex items-center gap-2 mt-4">
              ...
              <span className="text-sm font-bold text-gray-400">{product.score}/100</span>
            </div>

            <div className="mt-6 flex gap-4">
              {product.score >= 70 ? (
                <div className="flex items-center gap-2 text-green-600 bg-green-50 px-3 py-2 rounded-xl flex-1 justify-center font-bold text-sm">
                  <CheckCircle2 size={18} /> Recommended
                </div>
              ) : product.score >= 40 ? (
                <div className="flex items-center gap-2 text-yellow-600 bg-yellow-50 px-3 py-2 rounded-xl flex-1 justify-center font-bold text-sm">
                  <AlertTriangle size={18} /> Moderate Choice
                </div>
              ) : (
                <div className="flex items-center gap-2 text-red-600 bg-red-50 px-3 py-2 rounded-xl flex-1 justify-center font-bold text-sm">
        ...
        <h3 className="text-lg font-bold mb-4 px-2">Sustainability Impact</h3>
        <div className="grid grid-cols-2 gap-4 mb-8">
          <Card className="flex flex-col gap-2 p-4">
            ...
            <p className="text-[10px] text-gray-400 font-medium">{product.details.carbonValue}</p>
          </Card>
          ...
          <Card className="flex flex-col gap-2 p-4">
            ...
            <p className="text-[10px] text-gray-400 font-medium">{product.details.plasticValue}</p>
          </Card>
          ...
          <Card className="flex flex-col gap-2 p-4">
            ...
            <p className="text-[10px] text-gray-400 font-medium">{product.details.ethicsValue}</p>
          </Card>
        </div>
        ...
        {recommendedAlts.length > 0 && (
          <section className="mb-8">
            <div className="flex justify-between items-center mb-4 px-2">
              <h3 className="text-lg font-bold">Better Alternatives</h3>
            ...
            <div className="space-y-4">
              {recommendedAlts.map((alt) => (
                <Card 
                  key={alt.id} 
                  className="flex items-center gap-4 border-2 border-primary/20 bg-green-50/30"
                  onClick={() => navigate(`/product/${alt.id}`)}
                >
                  <div className="w-20 h-20 rounded-xl overflow-hidden bg-white p-2">
                    <img src={alt.image} alt={alt.name} className="w-full h-full object-contain" />
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-sm leading-tight mb-1">{alt.name}</h4>
                  ...
                  <ChevronLeft className="rotate-180 text-gray-300 mr-2" />
                </Card>
              ))}
            </div>
          </section>
